...
```mermaid
graph TD
    A[Get Draft from Harvester Cache] --> B[Read Original Post carefully]
    B --> C{Is the query a real problem?}
    C -- No --> D[Discard Draft]
    C -- Yes --> E[Edit Draft: Remove AI fluff & generic sentences]
    E --> F[Inject personal dev experience / human tone]
    F --> G[Add deep-link to specific lesson or prompt]
    G --> H[Declare ownership: 'I built this free tool...']
    H --> I[Post Reply on Reddit]
```
...
```diff
-{% block title %}RohithBuilds — AI Prompts{% endblock %}
+{% block title %}Free AI Prompt Library & Prompt Engineering Vault | RohithBuilds{% endblock %}
+{% block meta_description %}Explore a library of 220+ curated developer prompts. Copy, optimize, and organize system prompts for coding, debugging, and AI automation.{% endblock %}
```

#### B. Courses Hub / Catalog Page (`templates/learn.html`)

```diff
-{% block title %}Learn Python Free Online | 100 Lessons | Rohith Builds{% endblock %}
-{% block meta_description %}Learn Python free online with 100 bite-sized lessons, coding challenges, and interactive AI tutoring designed to take you from beginner to AI developer.{% endblock %}
+{% block title %}Learn AI Engineering & Python Free | Interactive Courses | RohithBuilds{% endblock %}
+{% block meta_description %}Master Python, API integrations, and AI agent building. Take structured visual coding courses with real-world projects and a 24/7 interactive AI tutor.{% endblock %}
```

#### C. Individual Course Detail Page (`modules/learn/templates/learn/course.html`)

```diff
-{% block title %}{{ course.title }} | Rohith Builds{% endblock %}
-{% block meta_description %}{{ course.description or 'Learn ' + course.title + ' through step-by-step practical lessons, coding challenges, and interactive AI tutoring.' }}{% endblock %}
+{% block title %}Free {{ course.title }} Course | Step-by-Step Tutorial | RohithBuilds{% endblock %}
+{% block meta_description %}Enroll in our free {{ course.title }} course. Master concepts via hands-on exercises, real-world coding projects, and dedicated 24/7 AI tutor guidance.{% endblock %}
```

#### D. Individual Lesson/Day Page (`modules/learn/templates/learn/lesson.html`)

```diff
-{% block title %}Day {{ day.day_number }}: {{ day.title }} | {{ course.title }} | Rohith Builds{% endblock %}
-{% block meta_description %}{{ day.short_description or 'Learn Day ' + day.day_number|string + ': ' + day.title + ' in the ' + course.title + ' course.' }}{% endblock %}
+{% block title %}{{ day.title }} (Day {{ day.day_number }}) | {{ course.title }} | RohithBuilds{% endblock %}
+{% block meta_description %}Master {{ day.title }} on Day {{ day.day_number }} of the {{ course.title }}. Learn with code challenges, visual guides, and interactive AI debugging.{% endblock %}
```
...
```mermaid
gantt
    title RohithBuilds User Acquisition Timeline
    dateFormat  YYYY-MM-DD
    section Phase 1: Foundation (Week 1-2)
    Implement SEO template fixes           :active, 2026-06-15, 7d
    Post Blog Article 1 (Python Roadmap)   :2026-06-18, 4d
    section Phase 2: Social Launch (Week 3-4)
    Launch Twitter Thread 1 (Visual)       :2026-06-25, 3d
    Reddit Harvester processing (1 reply/day) :2026-06-25, 14d
    section Phase 3: Scaling (Week 5+)
    Launch Twitter Thread 2 (AI Agent)     :2026-07-05, 3d
    Post Blog Article 2 (Build Agent)      :2026-07-10, 5d
    Outreach to university Discord channels:2026-07-15, 10d
```
